// ============================================================
//  admin/Orders.jsx  —  TransportHub
//  Modifications :
//   - Suppression TOTALE des MOCK_ORDERS
//   - Commandes réelles depuis GET /api/orders/
//   - Statuts corrects : pending, validated, assigned,
//     in_transit, delivered, cancelled
//   - Admin peut annuler une commande pending/validated
//   - Lien vers /admin/validation pour valider/assigner
...
import {
  ...
  MapPin, Flag, Map, Search, ChevronDown, ChevronUp,
  Package, RefreshCcw, ShieldCheck, Truck
} from 'lucide-react';

// ── Config statuts backend réels ────────────────────────────
const STATUS_CONFIG = {
  pending:    { label: 'En attente',  cls: 'badge-gray',   Icon: Clock          },
  validated:  { label: 'Validée',     cls: 'badge-orange', Icon: ShieldCheck     },
...
export default function AdminOrders() {
  ...
  const validateOrder = async (order) => {
    ...
    try {
      const res = await axios.post(`/api/orders/${order.id}/validate/`);
      setOrders(o => o.map(x => x.id === order.id ? res.data.order : x));
      toast.success(`Commande #${order.id} validée.`);
    } catch (err) {
  ...
  const cancelOrder = async (order) => {
    ...
    try {
      ...
      setOrders(o => o.map(x => x.id === order.id ? { ...x, status: 'cancelled' } : x));
      ...
      <div className="tp-page-header">
        ...
        <div className="tp-header-actions">
          ...
          <button className="tp-btn-primary" onClick={() => navigate('/admin/validation')}>
            <ShieldCheck size={15} /> Valider / Assigner
          </button>
      ...
      <div className="tp-filters">
        <div style={{ position: 'relative', flex: 1, minWidth: 220 }}>
          ...
          <input
            ...
            placeholder="Client, ville, transporteur, #commande…"
            value={search}
                ...
                <div className="tp-order-card-body">
                  <div className="tp-order-details-grid">
                    <div className="tp-od-item">
                      ...
                      <strong>{order.transporter_name || '—'}</strong>
                    </div>
                    <div className="tp-od-item">
                      <span>Véhicule</span>
                      <strong>
                    ...
                    <div className="tp-od-item">
                      <span>Priorité</span>
                      <strong>{order.priority || 'standard'}</strong>
                    </div>
                    <div className="tp-od-item">
                      <span>Créée le</span>
                  ...
                  <div className="tp-order-actions">
                    ...
                    {(order.status === 'pending' || order.status === 'validated') && (
                      <button
                        ...
                        onClick={() => navigate(`/admin/validation?order=${order.id}`)}
                      >
                    ...
                    {(order.status === 'pending' || order.status === 'validated') && (
                      ...
                      >
                        <XCircle size={14} />
                        {isActioning ? 'Annulation…' : 'Annuler'}
                      </button>
                    )}
                    ...
                    {(order.status === 'in_transit' || order.status === 'assigned') && (
                      <button
                        className="tp-btn-secondary"
                        onClick={() => navigate(`/admin/map?order=${order.id}`)}
                      >
                        <Map size={14} /> Suivre sur la carte
                      </button>
                    )}
                  </div>
                </div>
